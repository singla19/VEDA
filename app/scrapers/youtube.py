...
class YouTubeScraper:
    def __init__(self):
        ...
        if proxy_username and proxy_password:                   #Youtube blocks requests if a single IP makes too many transcripts request, so we use a proxy
            proxy_config = WebshareProxyConfig(
                proxy_username=proxy_username,
                proxy_password=proxy_password
            )
        
        self.transcript_api = YouTubeTranscriptApi(proxy_config=proxy_config)         #this is used to fetch transcripts(captions) for youtube videos
...
if __name__ == "__main__":
    ...
    channel_videos: List[ChannelVideo] = scraper.scrape_channel("UCn8ujwUInbJkBhffxqAPBVQ", hours=200)    #for now we are working with channel ID instead of username
